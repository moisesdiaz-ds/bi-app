...
#             df[c] = df[c].astype(str)
    ...
    if kcut:
        ...
        numeric_cols = (set(numeric_cols) - set(df[list(numeric_cols)].nunique()[df[list(numeric_cols)].nunique()<10].index )) - set([target])
        all_cols = set(df.columns) 
        categorical_cols = (all_cols - numeric_cols) 
        ...
        df = df[list(set(df.columns) - set(num_kcols))]
